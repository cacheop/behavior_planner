...
vector<Vehicle> Vehicle::choose_next_state(map<int, vector<Vehicle>> predictions) {
	...
	2. generate_trajectory(string state, map<int, vector<Vehicle>> predictions) - Returns a vector of Vehicle objects representing a vehicle trajectory, given a state and predictions. Note that trajectory vectors
	   might have size 0 if no possible trajectory exists for the state. 
	3. calculate_cost(Vehicle vehicle, map<int, vector<Vehicle>> predictions, vector<Vehicle> trajectory. Included from cost.cpp, computes the cost for a trajectory.
	*/
    ...
    for (int i = 0; i < possible_successor_states.size(); i++) {
        ...
        if (trajectory_for_state.size() != 0) {
            ...
            cout << "cost/state   =====>" << cost << "/" << state << endl;
            if (cost < min_cost) {
                min_cost = cost;
                next_state = possible_successor_states[i];
            }
        }
    }
    cout << "next=====>" << next_state << endl;
	return generate_trajectory(next_state, predictions);
}
